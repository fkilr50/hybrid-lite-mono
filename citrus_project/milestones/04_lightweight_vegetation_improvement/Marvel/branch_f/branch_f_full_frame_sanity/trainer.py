...
class Trainer:
    ...
    def configure_dataset_options(self):
        ...
        if not hasattr(self.opt, "sky_far_prior_weight"):
            self.opt.sky_far_prior_weight = 0.0
        if not hasattr(self.opt, "sky_far_prior_min_depth"):
            self.opt.sky_far_prior_min_depth = 40.0
        if not hasattr(self.opt, "sky_prior_top_fraction"):
            self.opt.sky_prior_top_fraction = 0.55
        if not hasattr(self.opt, "sky_prior_min_blue"):
            self.opt.sky_prior_min_blue = 0.25
        if not hasattr(self.opt, "sky_prior_blue_margin"):
            self.opt.sky_prior_blue_margin = 0.03
        if not hasattr(self.opt, "sky_prior_green_slack"):
            self.opt.sky_prior_green_slack = 0.08
        if not hasattr(self.opt, "sky_prior_min_brightness"):
            self.opt.sky_prior_min_brightness = 0.25
        if not hasattr(self.opt, "sky_prior_min_pixels"):
            self.opt.sky_prior_min_pixels = 200
        if not hasattr(self.opt, "sky_prior_require_lidar_invalid"):
            self.opt.sky_prior_require_lidar_invalid = False
        if not hasattr(self.opt, "sky_edge_contrast_weight"):
            self.opt.sky_edge_contrast_weight = 0.0
        if not hasattr(self.opt, "sky_edge_contrast_margin"):
            self.opt.sky_edge_contrast_margin = 0.015
        if not hasattr(self.opt, "sky_edge_min_pixels"):
            self.opt.sky_edge_min_pixels = 50
        if self.opt.max_train_steps < 0:
            raise ValueError(
        ...
        if self.opt.sky_far_prior_weight < 0:
            raise ValueError("--sky_far_prior_weight must be non-negative")
        if self.opt.sky_far_prior_min_depth <= 0:
            raise ValueError("--sky_far_prior_min_depth must be positive")
        if not 0 < self.opt.sky_prior_top_fraction <= 1:
            raise ValueError("--sky_prior_top_fraction must be in (0, 1]")
        if self.opt.sky_prior_min_pixels < 1:
            raise ValueError("--sky_prior_min_pixels must be at least 1")
        if self.opt.sky_edge_contrast_weight < 0:
            raise ValueError("--sky_edge_contrast_weight must be non-negative")
        if self.opt.sky_edge_contrast_margin < 0:
            raise ValueError("--sky_edge_contrast_margin must be non-negative")
        if self.opt.sky_edge_min_pixels < 1:
            raise ValueError("--sky_edge_min_pixels must be at least 1")
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            sanity_loss, sanity_stats = self.compute_full_frame_sanity_loss(
                inputs, outputs, scale, disp, color)
            if sanity_loss is not None:
                weighted_sanity_loss = sanity_loss / (2 ** scale)
                loss += weighted_sanity_loss
                losses["full_frame_sanity_loss/{}".format(scale)] = sanity_loss
                losses["full_frame_sanity_loss_weighted/{}".format(scale)] = weighted_sanity_loss
                for name, value in sanity_stats.items():
                    losses["{}/{}".format(name, scale)] = value
            total_loss += loss
    ...
    def compute_full_frame_sanity_loss(self, inputs, outputs, scale, disp, color):
        """Apply conservative RGB-derived priors outside LiDAR supervision.

        The goal is not to create fake ground truth everywhere. It only targets the
        known visual failure mode: sky predicted as near/ground-like and tree mass
        bleeding upward into sky regions.
        """
        if self.opt.sky_far_prior_weight <= 0 and self.opt.sky_edge_contrast_weight <= 0:
            return None, {}

        sky_mask = self.build_sky_mask(color)
        sky_mask = self.apply_lidar_invalid_filter(inputs, sky_mask, disp)
        stats = {"sky_prior_fraction": sky_mask.float().mean().detach()}

        total = torch.zeros((), device=disp.device)
        active_terms = 0

        if self.opt.sky_far_prior_weight > 0:
            depth_key = ("depth", 0, scale)
            depth_pred = outputs[depth_key] if depth_key in outputs else outputs[("depth", 0, 0)]
            if depth_pred.shape[-2:] != disp.shape[-2:]:
                depth_pred = F.interpolate(
                    depth_pred, disp.shape[-2:], mode="bilinear", align_corners=False)

            valid = sky_mask & torch.isfinite(depth_pred) & (depth_pred > 0)
            valid_count = int(valid.sum().item())
            stats["sky_far_valid_fraction"] = valid.float().mean().detach()
            if valid_count >= self.opt.sky_prior_min_pixels:
                target = torch.as_tensor(
                    self.opt.sky_far_prior_min_depth,
                    dtype=depth_pred.dtype,
                    device=depth_pred.device)
                pred = torch.clamp(depth_pred[valid], min=self.opt.lidar_loss_min_depth)
                far_loss = F.relu(torch.log(target) - torch.log(pred)).mean()
                total = total + self.opt.sky_far_prior_weight * far_loss
                stats["sky_far_prior_loss"] = far_loss.detach()
                active_terms += 1
            else:
                stats["sky_far_prior_loss"] = torch.zeros((), device=disp.device)

        if self.opt.sky_edge_contrast_weight > 0:
            upper_sky = sky_mask[:, :, :-1, :]
            lower_non_sky = ~sky_mask[:, :, 1:, :]
            edge_mask = upper_sky & lower_non_sky
            edge_count = int(edge_mask.sum().item())
            stats["sky_edge_fraction"] = edge_mask.float().mean().detach()
            if edge_count >= self.opt.sky_edge_min_pixels:
                sky_disp = disp[:, :, :-1, :][edge_mask]
                below_disp = disp[:, :, 1:, :][edge_mask]
                margin = torch.as_tensor(
                    self.opt.sky_edge_contrast_margin,
                    dtype=disp.dtype,
                    device=disp.device)
                contrast_loss = F.relu(margin - (below_disp - sky_disp)).mean()
                total = total + self.opt.sky_edge_contrast_weight * contrast_loss
                stats["sky_edge_contrast_loss"] = contrast_loss.detach()
                active_terms += 1
            else:
                stats["sky_edge_contrast_loss"] = torch.zeros((), device=disp.device)

        if active_terms == 0:
            return None, stats
        return total, stats

    def build_sky_mask(self, color):
        """Conservative RGB sky heuristic for Citrus orchard frames."""
        red = torch.clamp(color[:, 0:1, :, :], 0, 1)
        green = torch.clamp(color[:, 1:2, :, :], 0, 1)
        blue = torch.clamp(color[:, 2:3, :, :], 0, 1)
        brightness = (red + green + blue) / 3.0

        height = color.shape[-2]
        y = torch.linspace(0, 1, height, device=color.device, dtype=color.dtype).view(1, 1, height, 1)
        top_mask = y <= self.opt.sky_prior_top_fraction
        blue_mask = (
            (blue >= self.opt.sky_prior_min_blue)
            & (blue >= red + self.opt.sky_prior_blue_margin)
            & (blue >= green - self.opt.sky_prior_green_slack)
            & (brightness >= self.opt.sky_prior_min_brightness)
        )
        return top_mask & blue_mask

    def apply_lidar_invalid_filter(self, inputs, sky_mask, disp):
        """Optionally restrict sky prior to pixels not supervised by LiDAR labels."""
        if not self.opt.sky_prior_require_lidar_invalid:
            return sky_mask
        ...
        if "label_mask" in inputs:
            ...
            label_mask = inputs["valid_mask"]
        if label_mask is None:
            return sky_mask

        label_mask = label_mask.to(device=disp.device, dtype=disp.dtype)
        if label_mask.shape[-2:] != disp.shape[-2:]:
            label_mask = F.interpolate(label_mask, disp.shape[-2:], mode="nearest")
        return sky_mask & (label_mask <= 0)
