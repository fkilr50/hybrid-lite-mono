...
class LiteMonoOptions:
    def __init__(self):
        ...
        self.parser.add_argument("--disparity_smoothness",
                                 ...
                                 default=1e-3)
        self.parser.add_argument("--disable_photometric_loss",
                                 help="Branch C control: disable self-supervised RGB reprojection loss; "
                                      "keeps LiDAR supervision and explicit RGB edge-aware smoothness",
                                 action="store_true")
        ...
        self.parser.add_argument("--lidar_loss_min_valid_pixels",
                                 ...
                                 default=500)
        self.parser.add_argument("--sky_far_prior_weight",
                                 type=float,
                                 help="Branch F full-frame sanity loss: penalize sky-like pixels that predict too near; 0 disables",
                                 default=0.0)
        self.parser.add_argument("--sky_far_prior_min_depth",
                                 type=float,
                                 help="minimum desired depth in RGB-detected sky pixels",
                                 default=40.0)
        self.parser.add_argument("--sky_prior_top_fraction",
                                 type=float,
                                 help="only search for sky in this top fraction of the image",
                                 default=0.55)
        self.parser.add_argument("--sky_prior_min_blue",
                                 type=float,
                                 help="minimum blue channel value for the conservative sky heuristic",
                                 default=0.25)
        self.parser.add_argument("--sky_prior_blue_margin",
                                 type=float,
                                 help="required blue-over-red margin for sky heuristic",
                                 default=0.03)
        self.parser.add_argument("--sky_prior_green_slack",
                                 type=float,
                                 help="allowed amount that green may exceed blue in sky heuristic",
                                 default=0.08)
        self.parser.add_argument("--sky_prior_min_brightness",
                                 type=float,
                                 help="minimum RGB brightness for the sky heuristic",
                                 default=0.25)
        self.parser.add_argument("--sky_prior_min_pixels",
                                 type=int,
                                 help="minimum sky-prior pixels needed before applying sky far loss",
                                 default=200)
        self.parser.add_argument("--sky_prior_require_lidar_invalid",
                                 help="apply sky/far prior only where the LiDAR valid mask is inactive",
                                 action="store_true")
        self.parser.add_argument("--sky_edge_contrast_weight",
                                 type=float,
                                 help="Branch F anti-bleed loss: require non-sky below sky to have larger disparity",
                                 default=0.0)
        self.parser.add_argument("--sky_edge_contrast_margin",
                                 type=float,
                                 help="minimum normalized disparity jump from sky to non-sky below it",
                                 default=0.015)
        self.parser.add_argument("--sky_edge_min_pixels",
                                 type=int,
                                 help="minimum sky/tree transition pixels needed before applying contrast loss",
                                 default=50)
        ...
        self.parser.add_argument("--skip_optimizer_load",
                                 help="load model weights as initialization but start optimizer state fresh",
                                 action="store_true")
