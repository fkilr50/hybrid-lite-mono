...
def scaled_metrics(pred_scaled, dense, eval_mask, eval_min_depth, eval_max_depth):
    gt_eval = dense[eval_mask].astype(np.float64)
    pred_eval = np.clip(
...
def render_comparison(
    ...
    base_raw, _ = run_lite_mono_inference(rgb_path, dense.shape, baseline_model, False)
    phase_raw, _ = run_lite_mono_inference(rgb_path, dense.shape, phase2_model, False)
    ...
    base_scale = median_scale(base_raw, dense, eval_mask)
    phase_scale = median_scale(phase_raw, dense, eval_mask)
    if base_scale is None or phase_scale is None:
        raise ValueError("Invalid median scale for {}".format(row["rgb_rel"]))

    base_scaled = base_raw * base_scale
    phase_scaled = phase_raw * phase_scale
    base_error = np.full(dense.shape, np.nan, dtype=np.float32)
    phase_error = np.full(dense.shape, np.nan, dtype=np.float32)
    base_error[eval_mask] = np.abs(base_scaled[eval_mask] - dense[eval_mask])
    phase_error[eval_mask] = np.abs(phase_scaled[eval_mask] - dense[eval_mask])
    error_delta = phase_error - base_error
    label_display = np.where(eval_mask, dense, np.nan)

    base_metrics = scaled_metrics(
        base_scaled, dense, eval_mask, eval_min_depth, eval_max_depth
    )
    phase_metrics = scaled_metrics(
        phase_scaled, dense, eval_mask, eval_min_depth, eval_max_depth
    )

    depth_values = np.concatenate(
        [
            label_display[np.isfinite(label_display)].reshape(-1),
            base_scaled[eval_mask].reshape(-1),
            phase_scaled[eval_mask].reshape(-1),
        ]
    )
    ...
    err_max = percentile_or(
        np.concatenate([base_error[np.isfinite(base_error)], phase_error[np.isfinite(phase_error)]]),
        98,
        1.0,
    )
    if err_max <= 0:
    ...
    index = int(row["index"])
    output_path = output_dir / "{}_index_{:04d}_comparison.png".format(
        selected.role, index
    )
    ...
    fig.suptitle(
        "{} sample index {} | {} a1={:.3f}, abs_rel={:.3f} | {} a1={:.3f}, abs_rel={:.3f}".format(
            selected.role.title(),
            index,
            baseline_name,
            base_metrics["a1"],
            base_metrics["abs_rel"],
            phase2_name,
            phase_metrics["a1"],
            phase_metrics["abs_rel"],
        ),
        fontsize=13,
    )
    ...
    im = axes[0, 2].imshow(base_scaled, cmap="magma_r", vmin=vmin, vmax=vmax)
    axes[0, 2].set_title("{} median-scaled".format(baseline_name))
    fig.colorbar(im, ax=axes[0, 2], fraction=0.046, pad=0.04)
    im = axes[0, 3].imshow(phase_scaled, cmap="magma_r", vmin=vmin, vmax=vmax)
    axes[0, 3].set_title("{} median-scaled".format(phase2_name))
    ...
    im = axes[1, 1].imshow(base_error, cmap="inferno", vmin=0.0, vmax=err_max)
    axes[1, 1].set_title("{} abs error".format(baseline_name))
    fig.colorbar(im, ax=axes[1, 1], fraction=0.046, pad=0.04)
    im = axes[1, 2].imshow(phase_error, cmap="inferno", vmin=0.0, vmax=err_max)
    axes[1, 2].set_title("{} abs error".format(phase2_name))
    ...
    axes[1, 3].set_title("Error delta: Phase2 - baseline")
    ...
    return {
        ...
        "baseline_scale_ratio": base_scale,
        "phase2_scale_ratio": phase_scale,
        "baseline_median_scaled_abs_rel": base_metrics["abs_rel"],
        "baseline_median_scaled_a1": base_metrics["a1"],
        "phase2_median_scaled_abs_rel": phase_metrics["abs_rel"],
        "phase2_median_scaled_a1": phase_metrics["a1"],
        "phase2_minus_baseline_abs_rel": phase_metrics["abs_rel"] - base_metrics["abs_rel"],
        "phase2_minus_baseline_a1": phase_metrics["a1"] - base_metrics["a1"],
    }


def write_outputs(output_dir, summaries, split):
    json_path = output_dir / "{}_phase2_comparison_summary.json".format(split)
    csv_path = output_dir / "{}_phase2_comparison_summary.csv".format(split)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Render baseline-vs-Phase2 Citrus visual comparison panels.")
    parser.add_argument("--split", choices=["val", "test"], default="val")
    parser.add_argument("--phase2_results_dir", type=Path, required=True)
    parser.add_argument(
        "--dataset_workspace",
        type=Path,
    ...
    parser.add_argument("--phase2_weights", type=Path, required=True)
    ...
    parser.add_argument("--baseline_name", default="Plain Citrus")
    parser.add_argument("--phase2_name", default="Phase 2")
...
def main():
    args = parse_args()
    per_sample_path = (
        args.phase2_results_dir.resolve()
    ...
    phase2_model = load_lite_mono_model(
        args.phase2_weights.resolve(),
        args.model,
        args.no_cuda,
        args.min_depth,
        args.max_depth,
