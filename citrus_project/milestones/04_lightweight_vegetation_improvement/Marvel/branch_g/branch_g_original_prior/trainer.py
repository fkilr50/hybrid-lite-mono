...
class Trainer:
    def __init__(self, options):
        ...
        self.original_prior_models = None
        if self.opt.original_prior_weight > 0:
            self.load_original_prior_teacher()
    ...
    def configure_dataset_options(self):
        ...
        if not hasattr(self.opt, "original_prior_weight"):
            self.opt.original_prior_weight = 0.0
        if not hasattr(self.opt, "original_prior_weights_folder"):
            self.opt.original_prior_weights_folder = "weights/lite-mono"
        if not hasattr(self.opt, "original_prior_scales"):
            self.opt.original_prior_scales = [0]
        if not hasattr(self.opt, "original_prior_min_valid_pixels"):
            self.opt.original_prior_min_valid_pixels = 500
        if not hasattr(self.opt, "original_prior_require_lidar_invalid"):
            self.opt.original_prior_require_lidar_invalid = False
        if self.opt.max_train_steps < 0:
            raise ValueError(
        ...
        if self.opt.original_prior_weight < 0:
            raise ValueError("--original_prior_weight must be non-negative")
        if self.opt.original_prior_min_valid_pixels < 1:
            raise ValueError("--original_prior_min_valid_pixels must be at least 1")
        self.opt.original_prior_scales = [int(scale) for scale in self.opt.original_prior_scales]
        invalid_original_prior_scales = [
            scale for scale in self.opt.original_prior_scales if scale not in self.opt.scales
        ]
        if invalid_original_prior_scales:
            raise ValueError(
                "--original_prior_scales must be a subset of --scales; "
                "got invalid scales {}".format(invalid_original_prior_scales))
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if (
                self.opt.original_prior_weight > 0
                and scale in self.opt.original_prior_scales
                and self.original_prior_models is not None
            ):
                original_prior_loss, original_prior_stats = self.compute_original_prior_loss(
                    inputs, outputs, scale, disp)
                if original_prior_loss is not None:
                    weighted_original_prior_loss = (
                        self.opt.original_prior_weight * original_prior_loss / (2 ** scale))
                    loss += weighted_original_prior_loss
                    losses["original_prior_loss/{}".format(scale)] = original_prior_loss
                    losses["original_prior_loss_weighted/{}".format(scale)] = (
                        weighted_original_prior_loss)
                    losses["original_prior_valid_fraction/{}".format(scale)] = (
                        original_prior_stats["valid_fraction"])
                    losses["original_prior_mean_abs_delta/{}".format(scale)] = (
                        original_prior_stats["mean_abs_delta"])
    ...
    def load_original_prior_teacher(self):
        """Load frozen original Lite-Mono weights for Branch G prior preservation."""
        weights_folder = os.path.expanduser(self.opt.original_prior_weights_folder)
        if not os.path.isabs(weights_folder):
            weights_folder = os.path.join(str(REPO_ROOT), weights_folder)
        if not os.path.isdir(weights_folder):
            raise FileNotFoundError(
                "Cannot find --original_prior_weights_folder {}".format(weights_folder))

        teacher_encoder = networks.LiteMono(
            model=self.opt.model,
            drop_path_rate=0.0,
            width=self.opt.width,
            height=self.opt.height)
        teacher_depth = networks.DepthDecoder(teacher_encoder.num_ch_enc, self.opt.scales)
        ...
        if not os.path.isfile(encoder_path) or not os.path.isfile(depth_path):
            raise FileNotFoundError(
                "Original-prior teacher requires encoder.pth and depth.pth in {}".format(
                    weights_folder))

        encoder_state = torch.load(encoder_path, map_location=self.device)
        depth_state = torch.load(depth_path, map_location=self.device)
        teacher_encoder.load_state_dict({
            k: v for k, v in encoder_state.items()
            if k in teacher_encoder.state_dict()
        }, strict=False)
        teacher_depth.load_state_dict({
            k: v for k, v in depth_state.items()
            if k in teacher_depth.state_dict()
        }, strict=False)

        teacher_encoder.to(self.device).eval()
        teacher_depth.to(self.device).eval()
        for module in [teacher_encoder, teacher_depth]:
            for param in module.parameters():
                param.requires_grad = False

        self.original_prior_models = {
            "encoder": teacher_encoder,
            "depth": teacher_depth,
        }
        print("Loaded frozen original-prior teacher from {}".format(weights_folder))

    def compute_original_prior_loss(self, inputs, outputs, scale, student_disp):
        """Match original Lite-Mono relative disparity where LiDAR gives no target."""
        color = inputs[("color", 0, 0)]
        with torch.no_grad():
            teacher_features = self.original_prior_models["encoder"](color)
            teacher_outputs = self.original_prior_models["depth"](teacher_features)
            teacher_disp = teacher_outputs[("disp", scale)]
            if teacher_disp.shape[-2:] != student_disp.shape[-2:]:
                teacher_disp = F.interpolate(
                    teacher_disp,
                    student_disp.shape[-2:],
                    mode="bilinear",
                    align_corners=False)

        mask = torch.isfinite(student_disp) & torch.isfinite(teacher_disp)
        if self.opt.original_prior_require_lidar_invalid:
            label_mask = None
            if "label_mask" in inputs:
            ...
            if label_mask is not None:
                label_mask = label_mask.to(device=student_disp.device, dtype=student_disp.dtype)
                if label_mask.shape[-2:] != student_disp.shape[-2:]:
                    label_mask = F.interpolate(
                        label_mask,
                        student_disp.shape[-2:],
                        mode="nearest")
        ...
        losses = []
        deltas = []
        for batch_index in range(student_disp.shape[0]):
            image_mask = mask[batch_index:batch_index + 1]
            valid_count = int(image_mask.sum().item())
            if valid_count < self.opt.original_prior_min_valid_pixels:
                continue
            student_valid = torch.clamp(
                student_disp[batch_index:batch_index + 1][image_mask], min=1e-7)
            teacher_valid = torch.clamp(
                teacher_disp[batch_index:batch_index + 1][image_mask].detach(), min=1e-7)
            student_norm = student_valid / (student_valid.mean().detach() + 1e-7)
            teacher_norm = teacher_valid / (teacher_valid.mean() + 1e-7)
            delta = torch.abs(student_norm - teacher_norm)
            losses.append(delta.mean())
            deltas.append(delta.detach().mean())

        valid_fraction = mask.float().mean().detach()
        if not losses:
            zero = torch.zeros((), device=student_disp.device)
            return None, {"valid_fraction": valid_fraction, "mean_abs_delta": zero}

        prior_loss = torch.stack(losses).mean()
        return prior_loss, {
            "valid_fraction": valid_fraction,
            "mean_abs_delta": torch.stack(deltas).mean(),
        }
