...
$RunName = "branch_g_original_prior_from_b24_b12_2ep_w005_laptop"
...
& $Python citrus_project/milestones/04_lightweight_vegetation_improvement/Marvel/compare_phase2_visuals.py `
  ...
  --output_dir "$VisualDir/branch_b_w24_vs_branch_g_val" `
  --baseline_name "Branch B w24" `
  --phase2_name "Branch G 2ep" 2>&1 | Tee-Object -FilePath $PostprocessLog -Append
...
& $Python citrus_project/milestones/04_lightweight_vegetation_improvement/Marvel/compare_phase2_visuals.py `
  ...
  --baseline_weights $BaseWeights `
  --phase2_weights $WeightsFolder `
  --output_dir "$VisualDir/branch_b_w24_vs_branch_g_test" `
  --baseline_name "Branch B w24" `
  --phase2_name "Branch G 2ep" 2>&1 | Tee-Object -FilePath $PostprocessLog -Append
